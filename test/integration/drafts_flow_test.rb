...
class DraftsFlowTest < ActionDispatch::IntegrationTest
  ...
  def test_master_update
    patch_update_page('master', '5ca2eebcc4a00d1c95608c549fc39e287a462330')
  end

  def test_working_update
    patch_update_page(user, '5ca2eebcc4a00d1c95608c549fc39e287a462330')
  end

  def test_custom_update
    patch_update_page('custom', 'a9441d8aaaacb7a9f580b90a9edc6598c99bc877')
  end
  ...
  def patch_update_page(name_or_user, draft_sha1)
    sign_in user
    patch_via_redirect website_branch_draft_path(website, website.branch(name_or_user), draft_sha1)
    assert_response 200
    assert_equal edit_website_branch_draft_path(website, website.branch(name_or_user), draft_sha1), path
  end
end

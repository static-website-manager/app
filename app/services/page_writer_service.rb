class PageWriterService
  def self.perform(user, website, branch, page, page_content, commit_message)
    oid = nil
    ...
    begin
      ...
      oid = rugged_repository.write(page_content, :blob)
      index = rugged_repository.index
      index.read_tree(rugged_branch.target.tree)
      index.add(path: page.raw_pathname, oid: oid, mode: 0100644)
      ...
      Rugged::Commit.create(rugged_repository,
        author: author,
        committer: author,
        message: commit_message.present? ? commit_message : "Save changes to #{page.raw_pathname}",
